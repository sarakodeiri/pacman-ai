...
class LineByLineProblem (SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        myTup = tuple(map(tuple, self.foods))
        return (self.startingPosition, 'HI!', Directions.STOP, myTup) #####################DIRECTIONS.BLAH
        # util.raiseNotDefined()
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        is_there_any_food = False
        for col in state[3]:
            for row in col:
                if row:
                    is_there_any_food = True
        # for f in state[2][state[0][0]]:
        #     if f:
        #         is_there_any_food = True
        if not is_there_any_food and state[0] == self.goal:
            return True
        return False
        # util.raiseNotDefined()
    ...
    def getNextStates(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                nextState = (nextx, nexty)
                tmp = list(map(list, newFoodArr))
                tmp[nextx][nexty] = False
                newFoodArr = tuple(map(tuple, tmp))
                next_states.append((nextState, 'HI!', action, newFoodArr))
            is_there_any_food_in_col = False
            for f in state[3][state[0][0]]:
                if f:
                    is_there_any_food_in_col = True
            if is_there_any_food_in_col:
                for item in next_states:
                    if item[2] == Directions.EAST or item[2] == Directions.WEST:
                        next_states.remove(item)
            elif self.startingPosition[0] > self.goal[0]:
                for item in next_states:
                    if item[2] == Directions.EAST:
                        next_states.remove(item)
            elif self.startingPosition[0] < self.goal[0]:
                for item in next_states:
                    if item[2] == Directions.WEST:
                        next_states.remove(item)               
...
class ScaryProblem (PositionSearchProblem):
    """
        This search problem finds paths through map layout while avoiding dangerous zones
    ...
    def __init__(self, gameState, goal=(1, 1), start=None, warn=True, visualize=True):
        ...
        for ghost in self.ghosts:
            for i in range(ghost[0] - 1, ghost[0] + 2):
                for j in range(ghost[1] - 1, ghost[1] + 2):
                    self.allPreventedStates.append((i, j))
    ...
    def cost_function(self, state):
        ...
        if state in self.allPreventedStates:
            return float('inf')
        else:
